...
public class RandomArray {
    ...
    public static int[][] randomSquareArray(int n, int min, int max) { // n elements, max and min weight range
        ...
        } while (!Connected(arr));   //if random arr generated is not connected, keep generating
    ...
    public static void print2DArray(int[][] arr) {       //prints weighted directed edges
        for (int[] a : arr) {
            for (int b : a) {
    ...
    public static void main(String[] args) {
        ...
        //can loop if you want to generate more matrix
        int[][] arr = randomSquareArray(input_size, -10, 10);
        ...
        System.out.printf("%d edges, %d vertices,\n",countEdges(arr), input_size);
    }
}
